...
{
    float d = Linear01Depth(SAMPLE_DEPTH_TEXTURE_LOD(_FAKEDepthTexture, sampler_FAKEDepthTexture, UnityStereoTransformScreenSpaceTex(uv), 0));
...
float4 FragAO(VaryingsDefault i) : SV_Target
{
    ...
    float d = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_FAKEDepthTexture, sampler_FAKEDepthTexture, i.texcoordStereo));
