Shader "Hidden/PostProcessing/TemporalAntialiasing"
{
    HLSLINCLUDE
        ...
        float4 _FAKEDepthTexture_TexelSize;
        ...
        {
            const float2 k = _FAKEDepthTexture_TexelSize.xy;

            const float4 neighborhood = float4(
                SAMPLE_DEPTH_TEXTURE(_FAKEDepthTexture, sampler_FAKEDepthTexture, UnityStereoClamp(uv - k)),
                SAMPLE_DEPTH_TEXTURE(_FAKEDepthTexture, sampler_FAKEDepthTexture, UnityStereoClamp(uv + float2(k.x, -k.y))),
                SAMPLE_DEPTH_TEXTURE(_FAKEDepthTexture, sampler_FAKEDepthTexture, UnityStereoClamp(uv + float2(-k.x, k.y))),
                SAMPLE_DEPTH_TEXTURE(_FAKEDepthTexture, sampler_FAKEDepthTexture, UnityStereoClamp(uv + k))
            );
            ...
            float3 result = float3(0.0, 0.0, SAMPLE_DEPTH_TEXTURE(_FAKEDepthTexture, sampler_FAKEDepthTexture, uv));
